...
## Порядок вибору інструмента
У Codex спочатку використовувати штатні browser tools через CUA в окремій QA-сесії
для інтерактивної та візуальної перевірки. Цей skill застосовується як fallback,
якщо штатна автоматизація браузера недоступна, не дає ізольованого контексту або
конкретна перевірка потребує Playwright MCP.
Повторювані regression-тести в `tests/browser/` запускаються штатним Playwright CLI
проєкту й не замінюються ручною CUA-перевіркою.
