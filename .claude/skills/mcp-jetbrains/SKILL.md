...
`PHPSTORM_MCP_URL` або з локального fallback у `.mcp.json`; після перезапуску IDE порт
змінюється — тоді або експортувати `PHPSTORM_MCP_URL`, або синхронізувати конфіги
скриптом `bash scripts/sync-phpstorm-mcp-port.sh` З РЕПОЗИТОРІЮ ІНФРИ · він бере порт з
IDE й розкочує його на конфіги ВСІХ наборів одразу, тому запускати його в окремому
проєкті не треба.
...
**Порт дрейфує · перевіряти ПЕРЕД роботою, а не після збою.** IDE зберігає порт у
`options/mcpServer.xml` і міняє його при перезапуску, якщо попередній зайнятий, а
конфіги 15 наборів (`.mcp.json`, `.codex/config.toml`, `opencode.json`) тримають
його значенням за замовчуванням. Тому одна зміна порту мовчки вимикає інструмент
одразу в усіх клієнтів: Claude показує `ConnectionRefused`, Codex і OpenCode просто
не бачать інструментів. Живий випадок 2026-09-13 · у конфігах 64442, IDE слухала
64444, і жоден агент не міг узяти факт з IDE.

```bash
bash scripts/sync-phpstorm-mcp-port.sh --check   # розбіжність? код 1 і перелік файлів
bash scripts/sync-phpstorm-mcp-port.sh           # узяти порт з IDE і розкотити на всі набори
```

Це вже вбудовано в `agent-check.sh preflight`, тому побачити дрейф можна ще до
першої задачі. Після синхронізації клієнта треба перезапустити: MCP читається на
старті сесії. Разова альтернатива без правки файлів — експортувати
`PHPSTORM_MCP_URL` у профілі оболонки: усі конфіги читають саме цю змінну.

Обовʼязкове правило інспекцій (§12 довідника та правила проєкту) спирається саме на це: інспекції по
змінених файлах агент робить через `get_file_problems` з `errorsOnly: true`, а
...
результат навіть на файлі з явною помилкою, тобто фальшивий «0 ERROR» · це гірше за
відсутність перевірки, бо виглядає як доказ. Доведено канаркою 2026-09-12 у
`www/bdo_ua_translate`. Звідси й порядок доказу: перш ніж доповісти нуль, прогнати
інструмент на файлі з гарантованою помилкою, переконатись, що вона знайдена, і
прибрати канарку тим самим прогоном.

### Коли інспекція віддає `timedOut: true`
...
3. якщо й далі timeout — викликати `get_file_problems` без пакета: один файл не
   ділить спільний бюджет і відпрацьовує там, де пакетний виклик здається.

Після першого успішного розбору файл лишається в кеші індексу й відповідає одразу.
`get_file_problems` з `errorsOnly: false` на
